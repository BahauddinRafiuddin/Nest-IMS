...
import { exportTransactionReport, getAllTransactionReport } from "../../api/superAdmin.api";
...
const SuperAdminFinance = () => {
  ...
  const exportExcel = async () => {
    await exportTransactionReport(filters, "excel");
  };
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-6 border-b border-slate-200 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">
            Analyze and manage system-wide revenue flow.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-4xl border border-slate-200 p-6 shadow-sm overflow-hidden h-fit">
          ...
          <div className="space-y-4">
            {breakdown.map((b) => (
              <div
                key={b.commissionPercentage}
                className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between group hover:border-indigo-200 transition-colors"
              >
                <div>
                  ...
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                    {b.totalTransactions} Transactions
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-[10px] text-slate-400 font-medium tracking-tight">
                    Rev: ₹{b.totalRevenue.toLocaleString()}
                  </div>
                </div>
              </div>
            ))}
                ...
                {transactions.map((t) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      <div className="text-[10px] font-black text-indigo-500 bg-indigo-50 inline-block px-1.5 py-0.5 rounded-md mt-1 italic">
                        {t.commissionPercentage}%
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="text-sm font-bold text-emerald-600">
                        ₹{t.superAdminCommission.toLocaleString()}
                      </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="text-xs font-bold text-slate-500 flex items-center justify-end gap-1">
                        <Calendar size={12} />{" "}
...
const SummaryCard = ({ title, value, icon: Icon, color }) => {
  ...
  return (
    <div
      className={`relative overflow-hidden p-6 rounded-3xl shadow-xl ${themes[color]}`}
    >
